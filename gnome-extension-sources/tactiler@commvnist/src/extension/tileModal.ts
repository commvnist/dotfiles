...
const TILE_SELECTION_TIMEOUT_MS = 1000;
const MAX_STORED_MONITOR_INDEX = 5;

export const TileModal = GObject.registerClass(
    class TileModal extends St.Widget {
        _settings: Gio.Settings;
        _window: Meta.Window;
        _onHide: () => void;
        _onMove: (window: Meta.Window, area: Area) => void;
        ...
        constructor(
            ...
            window: Meta.Window,
            onHide: () => void,
            onMove: (window: Meta.Window, area: Area) => void,
        ) {
            super({
                reactive: true,
                visible: true,
            ...
            if (!this._grab) {
                this.destroy();
                throw new Error("Unable to acquire GNOME Shell modal grab");
            }
        ...
        onActivateTile(area: Area) {
            ...
            if (!lastArea || !lastDate || lastDate + TILE_SELECTION_TIMEOUT_MS < now) {
                this._lastArea = area;
                this._lastDate = now;
                return;
            }
        ...
        onNextMonitor(): void {
            ...
            this._monitor = (this._monitor + 1) % monitors;
            this.destroyTiles();
            this.createTiles();
        }
        ...
        onPrevMonitor(): void {
            ...
            this._monitor = (this._monitor - 1 + monitors) % monitors;
            this.destroyTiles();
            this.createTiles();
        }
        ...
        saveMonitorLayout(monitor: number, layout: number): void {
            if (monitor > MAX_STORED_MONITOR_INDEX) {
                return;
            }
            this._settings.set_int(`monitor-${monitor}-layout`, layout);
        }

        loadMonitorLayout(monitor: number): number {
            if (monitor > MAX_STORED_MONITOR_INDEX) {
                return 1;
            }
        ...
        bindKey(name: string, handler: () => void) {
...
export const Tile = GObject.registerClass(
    class Tile extends St.BoxLayout {
        constructor(area: Area, text: string, styles: Styles) {
            super({
                style_class: "tile",
                style:
